...
unless found
  ...
  puts "    - add 'uses' field to 'users' table whose data type is TEXT"
  puts "    - set 'uses' field as users don't run any VMs in all zones"
  ...
  if str[0] == ?y || str[0] == ?Y ||str[0] == ?\n
    ...
    begin
      ...
      db.execute('ALTER TABLE users ADD uses TEXT')
    ensure
      db.close
    end

    # set initial values to 'uses' field
    begin
      db = SQLite3::Database.new(dbfile)
    ...
    users.each do |user|
      ...
      begin
        db = SQLite3::Database.new(dbfile)
        users = db.execute("UPDATE users SET uses='#{uses_str}' WHERE id=#{user[0]}")
      ensure
        db.close
      end
    end
